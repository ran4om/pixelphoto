 ...
 */
function swallowMultiParamCsiEndingInR(sequence) {
...
function safeDebugSessionId(raw) {
    const s = (raw ?? '').trim() || 'pixelphoto-tui';
    const cleaned = s.replace(/[^a-zA-Z0-9._-]+/g, '_').slice(0, 64);
    return cleaned || 'pixelphoto-tui';
}
const DEBUG_SESSION_ID = safeDebugSessionId(process.env.PIXELPHOTO_TUI_DEBUG_SESSION);
const DEBUG_CURSOR_DIR = join(PACKAGE_ROOT, '.cursor');
const DEBUG_NDJSON_LOG = join(DEBUG_CURSOR_DIR, `debug-${DEBUG_SESSION_ID}.log`);
