...
export function shouldUseMouse(env: NodeJS.ProcessEnv = process.env): boolean {
  return readBooleanEnv(env.PIXELPHOTO_USE_MOUSE) ?? true;
}

export function shouldEnableMouseMovement(env: NodeJS.ProcessEnv = process.env): boolean {
